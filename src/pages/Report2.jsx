...
const Report2 = () => {
  const { locationName, dataType } = useParams();
  ...
  const fetchReport = async (type) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5001/api2/location-readings/${dataType}/${type}/${locationName}`
      );
    ...
    <div style={{ padding: '1.5rem', minHeight: '100vh', backgroundColor: '#f3f4f6' }}>
      <div style={{
        ...
        <h2 style={{ fontSize: '1.5rem', fontWeight: 'bold', marginBottom: '1rem' }}>
          {dataType.charAt(0).toUpperCase() + dataType.slice(1)} Report for {locationName}
        </h2>
          ...
          <>
              ...
              <>
                <div style={{ width: '100%', overflowX: 'auto', marginBottom: '2rem' }}>
                  ...
                  >
                    ...
                    <YAxis 
                      label={{ 
                        value: `${dataType.charAt(0).toUpperCase() + dataType.slice(1)} (${unit})`, 
                        angle: -90, 
                        position: 'insideLeft' 
                      }} 
                    />
                ...
                {climateData && (
                  <div style={{
                    ...
                    <h3 style={{ fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '1rem' }}>
                      {dataType.charAt(0).toUpperCase() + dataType.slice(1)} Analysis Report
                    </h3>
                      ...
                      {['morning', 'afternoon', 'evening'].map(time => (
                        <div key={time} style={{
                          ...
                          <div style={{ marginBottom: '0.25rem' }}>
                            Previous: {climateData.averages.previous[time]}{unit}
                          </div>
                          <div style={{ marginBottom: '0.25rem' }}>
                            Current: {climateData.averages.current[time]}{unit}
                          </div>
                          <div style={{
                            ...
                            {climateData.analysis.changes[time]}{unit}
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {!reportData.length && (
              <div style={{ textAlign: 'center', padding: '2rem', color: '#6b7280' }}>
                Select a report type to view the {dataType} data
              </div>
            )}
          </>
        )}
